  ...
  <!-- 顶栏 -->
							...
							<c:forEach items="${list}" var="course">
								<tr>
									<td>${course.courseid}</td>
							  		...
							  		<td>${course.type}</td>
									<c:choose>
										<c:when test="${course.score<60}"><td style="color:red">${course.score}<td></c:when>
										<c:otherwise><td>${course.score}<td></c:otherwise>
									</c:choose>
								</tr>
							</c:forEach>
					        </tbody>
				    ...
				    <div class="panel-footer">
				    	<c:if test="${pagingUI != null}">
					    	<nav style="text-align: center">
					    		<ul class="pagination">
								  	<li><a id="pagebotton1"  href="javascript:void(0);"   onclick="showSelectedCourse()">&laquo;上一页</a></li>
								  	<li><a>${pageNo}</a></li>
									<li><a id="pagebotton2"  href="javascript:void(0);"  onclick="showSelectedCourse()">下一页&raquo;</a></li>
	...
	<script type="text/javascript">
		function nextPage(){
			...
			window.location.href="student/showmarkedcourse?pageNo="+nPageNo ; //跳转
		}
		function lastPage(){
			var pageNo=document.getElementById("pageNo").value;//获得当前页
			...
			window.location.href="student/showmarkedcourse?pageNo="+lPageNo ;//跳转
		}
	</script>
  </body>
</html>
