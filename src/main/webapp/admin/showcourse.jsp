  ...
  <div class="container" id="content">
				...
				<div class="panel panel-default">
				    <div class="panel-heading">
						<div class="row">
							...
							<button class="btn btn-default col-md-2" style="margin-top: 20px" onClick="location.href='admin/addcoursepage'">
								添加课程信息
							...
							<c:forEach items="${list}" var="course">
								<tr>
									<td>${course.courseid}</td>
							  		...
							  		<td>${course.score}</td>
									<td>
										<button class="btn btn-default btn-xs btn-info" onClick="location.href='admin/editcoursepage?courseId=${course.courseid}'">修改</button>
										<button class="btn btn-default btn-xs btn-danger btn-primary" onClick="javascript:if(confirm('确认删除吗?'))window.location='admin/removecourse?id=${course.id}'">删除</button>
									</td>
								</tr>
							</c:forEach>
					        </tbody>
	...
	<script type="text/javascript">
		function nextPage(){
			...
			window.location.href="admin/showcourse?pageNo="+nPageNo ; //跳转
		}
		function lastPage(){
			var pageNo=document.getElementById("pageNo").value;//获得当前页
			...
			window.location.href="admin/showcourse?pageNo="+lPageNo ;//跳转
		}
		function getCourse(){
			var courseId = document.getElementById("num").value ;
			window.location.href="admin/getcourse?courseId="+courseId ;//跳转
		}
	</script>
  </body>
</html>
