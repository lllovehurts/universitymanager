		...
		<div class="row">
			...
			<div id="right">
				...
				<div class="panel-body">
					<form class="form-horizontal" role="form" action="admin/editteacher" id="editfrom" method="post" onsubmit="return validate()">
						<input type="hidden" name="id" value="${update[0].id}">
						<div class="form-group">
							<label for="inputEmail3" class="col-sm-2 control-label">教师号</label>
							<div class="col-sm-5">
								<input type="number" class="form-control" id="teacherId" name="teacherid" placeholder="请输入教师号" value="${update[0].teacherid}" onblur="idValidate()" readonly="readonly">
								<input type="hidden" id="oldTeacherId" value="${update[0].teacherid}">							
							</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-5">
								<input type="text" class="form-control" id="teacherName" name="teachername" placeholder="请输入教师名称" value="${update[0].teachername}" onblur="nameValidate()">
							</div>
						...
						<div class="form-group">
							    ...
							    <div class="col-sm-5">
								    <label class="checkbox-inline">
									   	<c:choose>
												<c:when test="${'男' eq update[0].sex}"><input type="radio" name="sex" value="男" checked>男</c:when>
												<c:otherwise><input type="radio" name="sex" value="男">男</c:otherwise>
										</c:choose>
									</label>
									<label class="checkbox-inline">
										<c:choose>
												<c:when test="${'女' eq update[0].sex}"><input type="radio" name="sex" value="女" checked>女</c:when>
						...
						<div class="form-group">
							    ...
							    <div class="col-sm-5">
								    <input type="date"  id="birth" name="birth" value="<fmt:formatDate value="${update[0].birth}" dateStyle="medium" pattern="yyyy-MM-dd" />" onblur="birthValidate()">
							    </div>
							    <span id="birthV"></span>
						</div>
						<div class="form-group">
							    ...
							    <div class="col-sm-5">
								    <input type="date"  id="entry" name="entry" value="<fmt:formatDate value="${update[0].entry}" dateStyle="medium" pattern="yyyy-MM-dd" />" onblur="entryValidate()">
							    </div>
							    <span id="entryV"></span>
					    </div>
						<div class="form-group">
								...
								<div class="col-sm-5">
									<select class="form-control" name="position">
										<c:choose>
												<c:when test="${'教师' eq update[0].position}"><option value="教师" selected="selected">教师</option></c:when>
												<c:otherwise><option value="教师">教师</option></c:otherwise>
										</c:choose>
										<c:choose>
												<c:when test="${'助教' eq update[0].position}"><option value="助教" selected="selected">助教</option></c:when>
												<c:otherwise><option value="助教">助教</option></c:otherwise>
										</c:choose>
										<c:choose>
												<c:when test="${'讲师' eq update[0].position}"><option value="讲师" selected="selected">讲师</option></c:when>
												<c:otherwise><option value="讲师">讲师</option></c:otherwise>
										</c:choose>
										<c:choose>
												<c:when test="${'副教授' eq update[0].position}"><option value="副教授" selected="selected">副教授</option></c:when>
												<c:otherwise><option value="副教授">副教授</option></c:otherwise>
										</c:choose>
										<c:choose>
												<c:when test="${'教授' eq update[0].position}"><option value="教授" selected="selected">教授</option></c:when>
							...
							<div class="col-sm-5">
								<select class="form-control" name="collegeid.collegeid">
									<c:forEach items="${collegeList}" var="item">
										<c:choose>
											<c:when test="${item.collegename eq update[0].collegeid.collegename}"><option value="${item.collegeid}" selected="selected">${item.collegename}</option></c:when>
	...
	<script type="text/javascript">
		...
		function idValidate(){
			if(isNullValidate("teacherId")){
				var old = document.getElementById("oldTeacherId").value ;
				var newTeacherId = document.getElementById("teacherId").value ;
				if(old != newTeacherId){
